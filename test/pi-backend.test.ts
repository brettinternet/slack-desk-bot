...
import { createResponseCollector, PiBackend } from "../src/pi-backend.ts";
...
describe("Pi session management", () => {
  test("opens the persisted session for a conversation", async () => {
    ...
    await backend.run({ conversationId: "thread", prompt: "continue" });
  ...
  test("bounds live sessions with LRU eviction", async () => {
    ...
    await backend.run({ conversationId: "one", prompt: "a" });
    await backend.run({ conversationId: "two", prompt: "b" });
  ...
  test("status does not create a session", async () => {
    ...
    expect(await backend.command("missing", "status")).toBe(
  ...
  test("cancels an active request without waiting for it to finish", async () => {
    let finish!: () => void;
    ...
    const running = backend.run({ conversationId: "thread", prompt: "work" });
    await Bun.sleep(0);
    expect(await backend.command("thread", "cancel")).toBe("Active request cancelled.");
