...
- Cancels the requester's active request when they send `cancel`; configured operators can cancel any active request in the conversation.
...
Use `!help` to see Slack examples and all supported commands. `!status` reports a live session's model, context usage when available, cumulative cost, and message count; inactive persisted sessions retain the shorter session summary. `!reset` and `!cancel` start a fresh session while retaining its previous transcript or stop its active request. Plain `cancel` also cancels an active request. By default, users can cancel only their own request. IDs in the optional `SLACK_OPERATOR_USER_IDS` setting can cancel any active request in the conversation; they must also appear in `SLACK_ALLOWED_USER_IDS`, and the cancellation reply and structured log identify the cancelling user. Commands are case-insensitive exact messages; an unsupported message beginning with `!` points back to `!help` instead of invoking the agent. In a channel, an allowlisted user must mention the bot to start a conversation. Further allowlisted human replies in that thread do not need a mention, including after a service restart when the conversation has a persisted session. Root channel messages and replies in unrelated threads are ignored.
...
`SLACK_ALLOWED_USER_IDS` is a required comma-separated allowlist of Slack member IDs. `SLACK_OPERATOR_USER_IDS` is an optional comma-separated subset whose members may cancel another user's active request in the same conversation. Requests from all other users are rejected before Pi runs; the rejection reply is sent at most once per user and conversation every ten minutes so repeated messages cannot generate Slack API traffic. The app receives public and private channel message events so it can accept natural follow-ups, but it ignores channel roots and threads that an allowlisted mention has not claimed and that have no persisted SlackDeskBot session. Find a member ID in Slack from **Profile → More → Copy member ID**.
