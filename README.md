...
Set `SLACK_GITHUB_REPOS=work-org/repo,work-org/other` to the exact approved organization repositories (no wildcards). Remove any previous `SLACK_GITHUB_TOKEN_FILE` setting; it is no longer used. Without the repository allowlist, GitHub watches are not offered. Install `gh` and authenticate it for `github.com` **as the same OS user and with the same `GH_CONFIG_DIR`/home used by the service** (keep `GH_CONFIG_DIR` outside `SLACK_AGENT_CWD`) (`gh auth login`, then `gh auth status --hostname github.com`). The service checks CLI authentication at startup and uses `gh api` for read-only lookups. It disables interactive prompts and ignores inherited token environment overrides so it uses that saved login. A headless service cannot rely on a different user's interactive `gh` session; for Compose, the image includes `gh`, but you must provide an authenticated `gh` config accessible to the container's `bun` user (the default Compose file does not mount one). No `gh` command or credential is exposed to Pi. **The `gh` login may have access to personal repositories**; only the app-level allowlist and organization check restrict what these watch adapters request. This is not credential-level isolation. Existing watches remain stored but are not polled if `SLACK_GITHUB_REPOS` is removed.
